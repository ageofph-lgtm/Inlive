// ─────────────────────────────────────────────────────────────────────────────
//  GANTT CHART — Timeline slide — layout inspirado na referência visual
//  Coluna NS fixa à esquerda · Barras à direita · Linha TODAY vertical
// ─────────────────────────────────────────────────────────────────────────────
export default function GanttChart({ machines, D }) {
  ...
  for (const m of machines) {
    ...
    rawBlocks.push({ m, pi, pf, isActive, isPrio, run, overrun, isRecon });
  }

  // Sem data: mostrar também como linha sem barra
  const allRows = [];
  for (const m of machines) {
    const pi = m.previsao_inicio ? new Date(m.previsao_inicio) : null;
    const pf = m.previsao_fim    ? new Date(m.previsao_fim)    : null;
  ...
  const sorted = [...allRows].sort((a, b) => {
    // activas em curso primeiro, depois por data, depois sem data
  ...
  const COL_W = 130; // px — largura da coluna NS
  const ROW_H = 28;  // px — altura de cada linha
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden",gap:0}}>
      ...
      {/* ── HEADER: coluna NS + régua de dias ─────────────────────────── */}
      <div style={{display:"flex",flexShrink:0,height:38,
        ...
        {/* célula vazia sobre coluna NS */}
        <div style={{width:COL_W,flexShrink:0,borderRight:`1px solid ${D.dark?"rgba(255,255,255,0.08)":"rgba(0,0,0,0.08)"}`}}/>
        {/* régua dias */}
          ...
          {ruleDays.map((d,i) => {
            ...
            return (
              <div key={i} style={{
                ...
                width:(100/numDays)+"%", height:"100%",top:0,
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
        {rows.map((b, ri) => {
          ...
          return (
            <div key={b.m.id} style={{
              ...
              background: isOdd
                ? (D.dark?"rgba(255,255,255,0.018)":"rgba(0,0,0,0.018)")
                : "transparent",
            }}>
              ...
              <div style={{flex:1,position:"relative",overflow:"hidden"}}>
                ...
                {nowPct>=0&&nowPct<=100&&(
                  <div style={{
                    ...
                    left:`calc(${nowPct}% - 1px)`,width:"2px",
                    background:D.dark
                      ?"linear-gradient(180deg,rgba(200,16,46,0.9),rgba(200,16,46,0.5))"
                      :"rgba(200,16,46,0.7)",
                ...
                {/* barra da máquina */}
                {b.hasDate&&leftR!==null&&rightR!==null&&rightR>0&&leftR<100&&(
                  <div
                    title={`${b.m.serie} · ${fmtD(b.pi)} → ${fmtD(b.pf)}`}
                    style={{
                      position:"absolute",
                      left:leftR+"%", width:wPct+"%",
        ...
        {sorted.length > MAX_VISIBLE && (
          <div style={{
            textAlign:"center",fontFamily:FONT,fontSize:"8px",letterSpacing:"0.12em",
            color:D.dark?"rgba(255,255,255,0.25)":"rgba(0,0,0,0.3)",
            padding:"6px 0",
          }}>
